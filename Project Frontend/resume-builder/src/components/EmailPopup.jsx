  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const pdfBlob = await generatePDFBlob(
        ...
        { quality: 0.8, scale: 1.6 }
      );

      const sizeMB = (pdfBlob.size / (1024 * 1024)).toFixed(2);
      if (pdfBlob.size > 18 * 1024 * 1024) {
        toast.error(
          `Generated PDF is too large (${sizeMB} MB). Try reducing content or switching template/colors.`,
          { id: "email-send" }
        );
        setIsLoading(false);
        return;
      ...
      // 🔥 FIX: if request succeeds, email is sent
      await axiosInstance.post(API_PATHS.EMAIL.SEND_RESUME, formData , {suppressToast: true}
      );
